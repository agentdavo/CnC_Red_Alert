...
/* C11 replacements for SUPPORT.ASM and 2SUPPORT.ASM */

void Fat_Put_Pixel(long x, long y, long color, long size, void *page)
{
    LOG_CALL("Fat_Put_Pixel C version\n");
    if (!page || size <= 0)
        return;
...
{
    LOG_CALL("Conquer_Build_Fading_Table C version\n");
    if (!palette || !dest)
        return dest;
    if (frac > 255)
        frac = 255;
...
{
    LOG_CALL("Remove_From_List C version\n");
...
long Get_EAX(void)
{
    LOG_CALL("Get_EAX C version\n");
    return 0;
}

unsigned Square_Root(unsigned val)
{
    LOG_CALL("Square_Root C version\n");
